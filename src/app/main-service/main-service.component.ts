...
export class MainServiceComponent implements OnInit {
  ...
  getViewData() {
    ...
    this.spinner.show();
    this.service.postApi('/api/v1/admin/getMainServiceList', apireq, 0).subscribe((success: any) => {
  ...
  editCategory(x) {
     ...
     this.id = x._id,
      this.englishName = x.englishName,
      this.image=x.image
  ...
  editCategoryApi() {
    ...
    formData.append("englishName", this.editCateoryForm.value.englishName)
    ...
    this.service.formdataApi('/api/v1/admin/updateMainService', formData).subscribe(success => {
      if (success.body.response_code == 200) {
        ...
        this.formdata.delete("englishName")
        this.formdata.delete("serviceId")
        this.image = ''
