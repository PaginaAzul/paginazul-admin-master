...
export class ActionManagementComponent implements OnInit {
  ...
  fromMaxDate(event) {
    ...
    this.serviceCaller.postApi('/api/v1/admin/getActionList',apireq,1).subscribe((success:any)=>{
  ...
  getList() {
    ...
    this.spinner.show();
    this.serviceCaller.postApi("/api/v1/admin/getActionList", this.temp, 0).subscribe((data:any) => {
  ...
  deleteApi() {
    ...
    this.spinner.show();
    this.serviceCaller.postApi("/api/v1/admin/deleteAction", deleteData, 0).subscribe((data:any) => {
      if(data.response_code == 200) {
        this.spinner.hide();
        this.getList();
