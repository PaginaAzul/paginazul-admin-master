...
export class StoryComponent implements OnInit {
  ...
  getViewData() {
    ...
    this.spinner.show();
    this.service.postApi('/api/v1/admin/getCustomerStoryList', apireq, 0).subscribe((success: any) => {
  ...
  deleteApi() {
    ...
    this.service.postApi("/api/v1/admin/deleteCustomerStory", deleteData, 0).subscribe((data: any) => {
  ...
  addCategoryApi() {
    ...
    formData.append("username", this.addCategoryForm.value.username)
    formData.append("story", this.addCategoryForm.value.story)
    formData.append(`image`, this.uploadFile[0])
  

    this.service.formdataApi('/api/v1/admin/addCustomerStory', formData).subscribe(success => {
  ...
  editCategoryApi() {
    ...
    let formData = new FormData()
    formData.append("storyId", this.id)
    formData.append("resAndStoreId", this.editCateoryForm.value.resAndStoreId)
    formData.append("username", this.editCateoryForm.value.username)
    formData.append("story", this.editCateoryForm.value.story)
    if(this.uploadFile.length>0){
      formData.append(`image`, this.uploadFile[0])
    ...
    this.service.formdataApi('/api/v1/admin/updateCustomerStory', formData).subscribe(success => {
      if (success.body.response_code == 200) {
        this.spinner.hide();
        this.getViewData()
        this.editCateoryForm.reset();
