...
@Component({
  selector: 'app-professional-worker-detail',
  templateUrl: './professional-worker-detail.component.html',
  styleUrls: ['./professional-worker-detail.component.css']
})
export class ProfessionalWorkerDetailComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.editEarningForm = new FormGroup({
      totalEarning: new FormControl('', [Validators.required]),
    })

    this.dataSource = {
      chart: {
        caption: 'Total Own Professional Worker Earning',
        subCaption: 'Only Include Individual Earning',
        xAxisName: 'Period',
        yAxisName: 'Earning (k)',
        numberSuffix: 'K',
        theme: 'fusion'
  ...
  getUserData(){
    ...
    this.service.postApi('/api/v1/admin/getUserDetail',apireq,1).subscribe((success)=>{
      ...
      if(success.response_code==200){
        ...
        this.image = success.Data.professionalProfile
        this.categoryArray = success.Data.categoryNameArray
        this.subcategoryArray = success.Data.subCategoryNameArray
        this.workImageLength=success.Data.workeImage.length
  ...
  edit_earning_modal(x){

    this.totalEarning=x.totalEarning
    this.editEarningForm.patchValue({
  ...
  editEarning(data) {
    ...
    if (this.editEarningForm.invalid) {
      return
    }
    $('#edit_earning_modal').modal('hide')
