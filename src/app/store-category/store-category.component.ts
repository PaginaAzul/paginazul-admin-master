...
export class StoreCategoryComponent implements OnInit {
  ...
  getViewData() {
    ...
    this.spinner.show();
    this.service.postApi('/api/v1/admin/getStoreCategoryList', apireq, 0).subscribe((success: any) => {
  ...
  deleteApi() {
    ...
    let deleteData = {
      storeCategoryId: this.id,
    }

    this.service.postApi("/api/v1/admin/deleteStoreCategory", deleteData, 0).subscribe((data: any) => {
  ...
  addCategoryApi() {
    ...
    this.service.formdataApi('/api/v1/admin/addStoreCategory', formData).subscribe(success => {
  ...
  editCategoryApi() {
    ...
    formData.append("storeCategoryId", this.id)
    if(this.uploadFile.length>0){
      formData.append(`image`, this.uploadFile[0])
    ...
    this.service.formdataApi('/api/v1/admin/updateStoreCategory', formData).subscribe(success => {
      if (success.body.response_code == 200) {
        this.spinner.hide();
        this.editCateoryForm.reset();
        this.addCategoryForm.reset();
