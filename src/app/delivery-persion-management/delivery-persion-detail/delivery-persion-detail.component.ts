import { Component, OnInit } from '@angular/core';
import { Router, ActivatedRoute } from '@angular/router';
import { AppService } from 'src/app/app.service';
import { FormGroup, FormControl, Validators } from '@angular/forms';
declare var $;

@Component({
  selector: 'app-delivery-persion-detail',
  templateUrl: './delivery-persion-detail.component.html',
  styleUrls: ['./delivery-persion-detail.component.css']
})
export class DeliveryPersionDetailComponent implements OnInit {

  id: any;
  userDetail: any;
  image="data:image/jpeg;base64,"
  editEarningForm: any;
  totalEarning: any;
  dataSource: Object;
  title: string;
  viewImageOnModal: any;

  constructor(
    public router : Router,
    private activatedRoute: ActivatedRoute,
    public service :AppService
    ) { }

  ngOnInit() {
    this.getId()
    this.getUserData()

    this.editEarningForm = new FormGroup({
      totalEarning: new FormControl('', [Validators.required]),
    })

    this.dataSource = {
      chart: {
        caption: 'Total Own Delivery Worker Earning',
        subCaption: 'Only Include Individual Earning',
        xAxisName: 'Period',
        yAxisName: 'Earning (k)',
        numberSuffix: 'K',
        theme: 'fusion'
      },
      data: [
        { label: 'Yearly', value: '300' },
        { label: 'Monthly', value: '200' },
        { label: 'Weekly', value: '100' },
    
      ]
    };
  }
  plotRollOver($event){
    var dataValue = $event.dataObj.dataValue;
  }
  getId(){
    this.activatedRoute.params.subscribe(paramsId => {
      this.id = paramsId.id;
    });
  }

  getUserData(){
    let apireq={
      userId : this.id
    }
    this.service.postApi('/api/v1/admin/getUserDetail',apireq,1).subscribe((success)=>{
      if(success.response_code==200){
        this.userDetail = success.Data
        this.image = success.Data.deliveryPProfilePic
      }
      else{
        console.log(success.response_message)
      }
    },error=>{
      console.log("Something went wrong")
    })
  }

  edit_earning_modal(x){
    this.totalEarning=x.totalEarning
    this.editEarningForm.patchValue({
      totalEarning: x.totalEarning,
     
    })
  }

  editEarning(data) {

    if (this.editEarningForm.invalid) {
      return
    }
    $('#edit_earning_modal').modal('hide')
   
    let apireq = {
     
      adminId: localStorage.getItem('_id'),
      totalEarning:data.totalEarning
      
    }
    this.service.postApi('/api/v1/admin/passwordChange', apireq, 1).subscribe(success => {

      if (success.response_code == 200) {
      this.router.navigate(['/admin/dashboard'])
      } else {
        console.log(success.response_message)
      }
    }, error => {
      console.log("Something went wrong")
    })
  }

  viewImage(deliverPId1) {
    $('#image1').modal({ backdrop: 'static', keyboard: false });
    this.viewImageOnModal = deliverPId1
  }
  dismissmodal() {
    $('#image1').modal('hide')
  }

  reset(){
    this.editEarningForm.reset()
  }
}
